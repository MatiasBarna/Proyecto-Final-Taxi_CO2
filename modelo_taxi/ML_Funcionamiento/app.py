...
df_precios_electricos = pd.read_csv("data/Precios_Autos_Electricos.csv")
...
nuevos_datos_prediccion = pd.DataFrame(
    {'FastCharge_miH': [0], 'Efficiency_Whmi': [0], 'RapidCharge': [1], 'Range_Miles': [0]})

# Predicciones para las características basadas en el presupuesto del cliente
predicciones = modelo_rf.predict(
    nuevos_datos_prediccion[['FastCharge_miH', 'Efficiency_Whmi', 'RapidCharge', 'Range_Miles']])
...
if st.button("Confirmar Presupuesto"):
    ...
    if presupuesto_cliente < df_precios_electricos['PriceDollar'].min() - 5000 or presupuesto_cliente > \
            df_precios_electricos['PriceDollar'].max() + 5000:
...
df_taxi = pd.read_csv("data/Taxi_ML.csv")
    ...
    if predecir:
        ...
        if isinstance(resultado, dict):
            if 'error' in resultado:
                ...
                st.success(
                    f'La probabilidad de encontrar un viaje en la zona {Id_Zona} es {resultado["probabilidad_en_zona"]}')
